...
Halide::Runtime::Buffer<double, 2> fm_buf;
double fm[2001][5];
...
void setfm(void)
{
  ...
  /* fm is 2001 by 5.
   * the common access pattern is fm(x,0), then fm(x,1), then fm(x,2), then fm(x,3), ...
   * so, transpose the indexes so that the second index is contiguous in memory.
   */
  fm_buf = Halide::Runtime::Buffer<double>(2001, 5);
  //fm_buf = Halide::Runtime::Buffer<double>(5, 2001).transposed(0,1);
  delta  = 0.014;
  delo2  = delta * 0.5;
  rdelta = 1.0 / delta;
...
void  f0(double *f0val, double t) {
  ...
  *f0val   = fm[n][0] + x * (fm[n][1] + x * 0.5  * (fm[n][2] + x * (1./3.) * (fm[n][3] + x * 0.25 * fm[n][4])));
  return;
}
