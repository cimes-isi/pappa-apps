...
int rrreduc(int nm, int n, double **a, double **b, double *fv2) {
...
//     .......... pre-multiply by inv(l) and overwrite ..........
  ...
  for (i = j; i < n; i ++) {
    ...
    if (i != j)
       for (k = j; k < i; k ++) x = x - a[k][j] * b[i][k];

    if (j > 0) 
       for (k = 0; k < j; k ++) x = x - a[j][k] * b[i][k];
...
//        z contains the orthogonal transformation matrix produced in the reduction.
  ...
  for (i = 0; i < n; i++) {
      for (j = i; j < n; j++) z[j][i] = a[j][i];
      for (j = 0; j < i; j++) z[j][i] = 0.0;
      d[i] = a[n - 1][i];
  }
...
//     .......... scale row (algol tol then not needed) ..........
      ...
      if (scale == 0.0) {
         e[i] = d[i - 1];
         for (j = 0; j < i; j++) { d[j] = z[i - 1][j]; z[i][j] = 0.0; z[j][i] = 0.0; }
...
//     .......... form rrreduced a ..........
         for (j = 0; j < i; j++) {
             ...
             for (k = j; k < i; k++) z[k][j] = z[k][j] - f * e[k] - g * d[k];
...
//     .......... accumulation of transformation matrices ..........
   for (i = 1; i < n; i++) {
       ...
       if (h != 0.0) {
          ...
          for (k = 0; k < i; k++) d[k] = z[k][i] / h;

          for (j = 0; j < i; j++) {
             g = 0.0;
             for (k = 0; k < i; k++) g       = g + z[k][i] * z[k][j];
             for (k = 0; k < i; k++) z[k][j] = z[k][j] - g * d[k];
       }
       }

       for (k = 0; k < i; k++) z[k][i] = 0.0;
   }
...
int tql2(int nm, int n, double *d, double *e, double **z) {
//
...
void rrrebak(int nm, int n, double **b, double *dl, int m, double **z) {
//
...
//        z contains the transformed eigenvectors in its first m columns.
  ...
  for (j = 0; j < m; j++) {
      ...
      for (i = n - 1; i >= 0; i--) {
          x = z[i][j];
          for (k = i + 1; k < n; k++) x = x - b[k][i] * z[k][j];
          z[i][j] = x / dl[i];
} }   }


int rsg(int nm, int n, double **a, double **b, double *w, double **z, double *fv1, double *fv2) {
//
//     this subroutine calls the recommended sequence of subroutines from the eigensystem
//     subroutine package (eispack) to find the eigenvalues and eigenvectors (if desired)
...
//        fv1 and fv2 are temporary storage arrays.
  ...
  int ierr = rrreduc(nm, n, a, b, fv2);
  if (ierr != 0) return ierr;

// find both eigenvalues and eigenvectors
  tred2(nm, n, a, w, fv1, z);

  ierr = tql2(nm, n, w, fv1, z);
  if (ierr != 0) return ierr;

  rrrebak(nm, n, b, fv2, n, z);
  return ierr;
}


int rs(int nm, int n, double **a, double *w, double **z, double *fv1, double *fv2) {
//
//     this subroutine finds the eigenvalues and eigenvectors for the real symmetric matrix
//
//     on input
//
//        nm  must be set to the row dimension of the two-dimensional arrays.
...
void  Eigen_gen(double ** g_a, double ** g_s, double ** g_v) {
  int i, j;

//  Make local copies of g_a and g_s to preserve values and allocate scratch space
  double ** a    = (double **) malloc(nbfn * sizeof(double *));
  double ** s    = (double **) malloc(nbfn * sizeof(double *));
  double * fv1   = (double  *) malloc(nbfn * sizeof(double));
  double * fv2   = (double  *) malloc(nbfn * sizeof(double));
  double * evals = (double  *) malloc(nbfn * sizeof(double));

  for (i = 0; i < nbfn; i++) {
      a[i] = (double *) malloc(nbfn * sizeof (double));
      s[i] = (double *) malloc(nbfn * sizeof (double));
  ...
  int ierr = rsg(nbfn, nbfn, a, s, evals, g_v, fv1, fv2);
  if (ierr != 0) {printf("Error code %d\n", ierr); exit(1);}

  for (i = 0; i < nbfn; i++) { free(a[i]); free(s[i]); }
  free(a); free(s); free(fv1); free(fv2); free(evals);
}
...
//  Make local copy of g_a to preserve values and allocate scratch space
  ...
  for (i = 0; i < nbfn; i++) {
      a[i] = (double *) malloc(nbfn * sizeof (double));
      for (j = 0; j < nbfn; j ++) a[i][j] = g_a[i][j];
  }

  int ierr = rs(nbfn, nbfn, a, evals, g_v, fv1, fv2);
  if (ierr != 0) {printf("Error code %d\n", ierr); exit(1);}

  for (i = 0; i < nbfn; i++) free(a[i]);
  free(a); free(fv1); free(fv2); 
}
