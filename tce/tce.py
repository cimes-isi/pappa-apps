# Tensor Contraction Engine v.1.0.ETI
...
def ln():
   rv = '' # normal mode
   #rv = " // tce.py:" + str(sys._getframe(1).f_lineno) # uncomment to debug line generation
   return rv
...
#3/24
   ...
   def fortran77x(self,subroutinename="NONAME"):
      ...
      newline = string.join(["CALL GET_HASH_BLOCK(",arguments,")"],"")
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      ...
      for nindex in range(len(all)-1,-1,-1):
         ...
         if (arguments == ""):
            ...
            argumentsend = string.join([argumentsend,")"],"")
      arguments = string.join([arguments,argumentsend,")"],"")
      newline = string.join(["CALL PUT_HASH_BLOCK(",arguments,")"],"")
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
   ...
   def fortran77z(self,globaltargetindexes,types,subroutinename="NONAME"):
      ...
      # allocate offsets
      newcode.pointer = len(newcode.statements)
      newline = "IF (.not.MA_PUSH_GET(mt_int,2*length+1,'noname',l_a_offset,k_a_offset)) CALL ERRQUIT('" \
                +subroutinename+"',"+repr(errquit)+",MA_ERR)"
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      errquit = errquit + 1
      newline = "int_mb(k_a_offset) = length"
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      ...
      for nindex in range(len(all)-1,-1,-1):
         if (all[nindex].type == "hole"):
            ...
            boffset = "b - noab - 1"
         if (key == ""):
            key = all[nindex].show()+boffset
         else:
            if (all[nindex+1].type == "hole"):
               key = key + " + noab * ("+all[nindex].show()+boffset
            else:
               key = key + " + nvab * ("+all[nindex].show()+boffset
            keyend = keyend + ")"
      if (key == ""):
         key = "0"
      newline = "int_mb(k_a_offset+addr) = "+key+keyend
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      newline = "int_mb(k_a_offset+length+addr) = size"
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      newline = ""
      for index in all:
   ...
   def isoc99z(self,globaltargetindexes,types,subroutinename="NONAME"):
      """Precompute hash, hashsize, offsets, and filesize"""

      errquit = 0

      language = "ISOC99"
      newcode = Code(language,"OFFSET_"+subroutinename)
 
      newcode.add("headers",'/* Tensor:')
      newcode.add("headers",' * '+self.show())
      newcode.add("headers",' */' + ln())
 
      # declaration
      newint = "k_a_offset"
      newcode.add("integerarrays",newint)
      newcode.add("locals",newint)
      newcode.add("retvals",newint)
      newint = "size"
      newcode.add("integerarrays",newint)
      newcode.add("arguments",newint)
      newint = "length"
      ...
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      newcode.inserttiledforloops(globalsuper)
      newcode.inserttiledforloops(localsuper)
      newcode.inserttiledforloops(globalsub)
      newcode.inserttiledforloops(localsub)
      newcode.inserttiledifsymmetry(super,sub,self.irrep)
      newcode.inserttiledifrestricted(super+sub)
      newline = "length = length + 1;" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1

      # allocate offsets
      newcode.pointer = len(newcode.statements)
      newline = "k_a_offset = tce_int_malloc(2*length+1);" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      errquit = errquit + 1
      newline = "k_a_offset[0] = length;" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      ...
      if (key == ""):
         key = "0"
      newline = "k_a_offset[addr] = "+key+keyend+";" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      newline = "k_a_offset[length+addr] = *size;" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      newline = ""
      for index in all:
         if (newline == ""):
            newline = string.join(["*size += k_range[",index.show(),"b]"],"")
         else:
            newline = string.join([newline," * k_range[",index.show(),"b]"],"")
      if (newline == ""):
         newline = "*size = 1"
      newline = newline + ";" + ln()
      newcode.statements.insert(newcode.pointer,newline)
   ...
   def isoc99z_COUNTBLOCKS(self,globaltargetindexes,types,subroutinename="NONAME"):
      """Precompute hash, hashsize, offsets, and filesize"""

      errquit = 0

      language = "ISOC99"
      newcode = Code(language,"COUNTBLOCKS_"+subroutinename)
...
#           show = string.join([show, self.summation.tex()])
   ...
   def findthebestbreakdown(self,excitation=[],verbose=0,memory=0):
...
#              print stringbreakdowna + stringbreakdownb, maxoperationcost, "(discouraged)"
# ... temporary
         ...
         if (maxoperationcost[1]+maxoperationcost[2] < minoperationcost[1]+minoperationcost[2]):
            ...
            if (maxoperationcost[2] < minoperationcost[2]):
               if (not memory):
                  raise RuntimeError, "a slightly cheaper contraction order found"

      return breakdowns[ibest]

   def breakdown(self,label=-1,excitation=[],verbose=0,memory=0):
...
#     another = another.symmetrize(0)
      order = another.findthebestbreakdown(excitation,verbose,memory)
      if (label < 0):
         if (len(order) == 1):
            label = 0
         else:
...
class ListTensorContractions:
   ...
   def findthebestbreakdown(self,verbose=0,memory=0):
      """Iteratively calls findthebestbreakdown() for all tensor contractions"""
      for tensorcontraction in self.list:
         print tensorcontraction.show()
         tensorcontraction.findthebestbreakdown(self.excitationtensortypes(),verbose,memory)

   def breakdown(self,verbose=0,memory=0):
      ...
      for tensorcontraction in self.list:
         ...
         newchild = tensorcontraction.breakdown(label,self.excitationtensortypes(),verbose,memory)
   ...
   def pythongen(self,filename="NONAME"):
      """Generates python code for debugging purposes"""
      ...
      for tensorcontraction in self.list:
         ...
         newline = string.join([intermediate.pythongen(),"=",intermediate.pythongen(),"+",\
                                "(",tensorcontraction.factor.show(0),")"],"")
         for tensor in tensorcontraction.tensors:
      ...
      writetofile(pythoncode,string.join([filename,".py"],""))
...
class ElementaryTensorContraction:
   ...
   def tex(self,symbols={},verbose=1):
      """Returns a LaTeX string of the content"""
      show = string.join([self.tensors[0].tex(),"&="])
...
#           show = string.join([show, self.summation.tex()])
   ...
   def fortran77(self,globaltargetindexes,types,subroutinename="NONAME",fuse=0):
...
#3/24    superrangeone = range(len(self.tensors[1].indexes)/2)
#3/24    subrangeone = range(len(self.tensors[1].indexes)/2,len(self.tensors[1].indexes))
#3/24 else:
#3/24    superrangeone = range(len(self.tensors[1].indexes)/2,len(self.tensors[1].indexes))
#3/24    subrangeone = range(len(self.tensors[1].indexes)/2)
...
#3/24       superrangetwo = range(len(self.tensors[2].indexes)/2)
#3/24       subrangetwo = range(len(self.tensors[2].indexes)/2,len(self.tensors[2].indexes))
#3/24    else:
#3/24       superrangetwo = range(len(self.tensors[2].indexes)/2,len(self.tensors[2].indexes))
#3/24       subrangetwo = range(len(self.tensors[2].indexes)/2)
#3/24
...
#4/17/04 from here (loop fusion)
      if (fuse):
         newcode.add("logicals","skipped")
         newline = "skipped = .true."
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         ...
         for npermutation in range(len(currentfactor.permutations)):
            ...
            for nindex in range(len(permutedindexes)):
               indexa = "t_" + self.tensors[0].indexes[nindex].show() + "b"
               indexb = permutedindexes[nindex].show() + "b"
               newline = newline + conjugation + "("+indexa+" .eq. "+indexb+")"
               conjugation = " .and. "
            newline = newline + ") skipped = .false."
            newcode.statements.insert(newcode.pointer,newline)
            newcode.pointer = newcode.pointer + 1
...
#4/17/04 ... to here
      ...
      for index in superglobalzero+superlocalzero+subglobalzero+sublocalzero:
         if (newline == ""):
            newline = "dimc = int_mb(k_range+"+index.show()+"b-1)"
         else:
            newline = newline+" * int_mb(k_range+"+index.show()+"b-1)"
      if (newline == ""):
         newline = "dimc = 1"
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      if (three):
         ...
         newline = string.join(["IF (.not.MA_PUSH_GET(mt_dbl,dimc,'noname',l_c_sort,k_c_sort)) CALL ERRQUIT('",\
                                subroutinename,"',",repr(errquit),",MA_ERR)"],"")
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         newline = "CALL DFILL(dimc,0.0d0,dbl_mb(k_c_sort),1)"
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
...
#4/15/04    newint = string.join([index.show(),"b"],"")
#4/15/04    newline = string.join([newline,conjugation,"int_mb(k_spin+",newint,"-1)"],"")
#4/15/04    conjugation = "+"
#4/15/04 newline = string.join([newline,".eq.",repr(2*len(indexesone)),")) THEN"],"")
#4/15/04 newcode.statements.insert(newcode.pointer,newline)
#4/15/04 newcode.pointer = newcode.pointer + 1
#4/15/04 for index in indexesone:
#4/15/04    newint = string.join([index.show(),"b_1"],"")
#4/15/04    newcode.add("integers",newint)
#4/15/04    newline = string.join([newint," = int_mb(k_alpha+",index.show(),"b-1)"],"")
#4/15/04    newcode.statements.insert(newcode.pointer,newline)
#4/15/04    newcode.pointer = newcode.pointer + 1
...
#4/15/04    newint = string.join([index.show(),"b_1"],"")
#4/15/04    newcode.add("integers",newint)
#4/15/04    newline = string.join([newint," = ",index.show(),"b"],"")
#4/15/04    newcode.statements.insert(newcode.pointer,newline)
#4/15/04    newcode.pointer = newcode.pointer + 1
...
#4/15/04       newline = string.join([newline,conjugation,"int_mb(k_spin+",newint,"-1)"],"")
#4/15/04       conjugation = "+"
#4/15/04    newline = string.join([newline,".eq.",repr(2*len(indexestwo)),")) THEN"],"")
#4/15/04    newcode.statements.insert(newcode.pointer,newline)
#4/15/04    newcode.pointer = newcode.pointer + 1
#4/15/04    for index in indexestwo:
#4/15/04       newint = string.join([index.show(),"b_2"],"")
#4/15/04       newcode.add("integers",newint)
#4/15/04       newline = string.join([newint," = int_mb(k_alpha+",index.show(),"b-1)"],"")
#4/15/04       newcode.statements.insert(newcode.pointer,newline)
#4/15/04       newcode.pointer = newcode.pointer + 1
...
#3/24/2004 end
            ...
            for nindex in range(len(permutedindexes)-1,-1,-1):
               ...
               if (arguments == ""):
                  ...
                  argumentsend = string.join([argumentsend,")"],"")
            if (not arguments):
               arguments = "d_a,dbl_mb(k_a),dima,int_mb(k_a_offset),0"
            else:
               arguments = string.join([arguments,argumentsend,")"],"")
            newline = string.join(["CALL GET_HASH_BLOCK(",arguments,")"],"")
            newcode.statements.insert(newcode.pointer,newline)
...
#4/6/04        newline = string.join(["DO ",newint," = 1,int_mb(k_range+",newint,"b-1)"],"")
...
#4/6/04        if (newline == ""):
#4/6/04           newline = string.join(["idima = ",permutedindexes[nindex].show()],"")
#4/6/04        else:
#4/6/04           newline = string.join([newline," + int_mb(k_range+",permutedindexes[nindex+1].show(),"b-1)"\
#4/6/04                                  " * ((",permutedindexes[nindex].show()," - 1)"],"")
#4/6/04           newlineend = string.join([newlineend,")"],"")
#4/6/04     newline = string.join([newline,newlineend],"")
...
#4/6/04        if (newline == ""):
#4/6/04           newline = string.join(["idima_sort = ",sorted[nindex].show()],"")
#4/6/04        else:
#4/6/04           newline = string.join([newline," + int_mb(k_range+",sorted[nindex-1].show(),"b-1)"\
...
#3/24/2004 end
               ...
               for nindex in range(len(permutedindexes)-1,-1,-1):
                  ...
                  if (arguments == ""):
                     ...
                     argumentsend = string.join([argumentsend,")"],"")
               if (not arguments):
                  arguments = "d_b,dbl_mb(k_b),dimb,int_mb(k_b_offset),0"
               else:
                  arguments = string.join([arguments,argumentsend,")"],"")
               newline = string.join(["CALL GET_HASH_BLOCK(",arguments,")"],"")
               newcode.statements.insert(newcode.pointer,newline)
...
#4/6/04        for nindex in range(len(permutedindexes)-1,-1,-1):
#4/6/04           if (newline == ""):
#4/6/04              newline = string.join(["idimb = ",permutedindexes[nindex].show()],"")
#4/6/04           else:
#4/6/04              newline = string.join([newline," + int_mb(k_range+",permutedindexes[nindex+1].show(),"b-1)"\
#4/6/04                                     " * ((",permutedindexes[nindex].show()," - 1)"],"")
#4/6/04              newlineend = string.join([newlineend,")"],"")
...
#4/6/04        for nindex in range(len(sorted)):
#4/6/04           if (newline == ""):
#4/6/04              newline = string.join(["idimb_sort = ",sorted[nindex].show()],"")
#4/6/04           else:
#4/6/04              newline = string.join([newline," + int_mb(k_range+",sorted[nindex-1].show(),"b-1)"\
#4/6/04                                     " * ((",sorted[nindex].show()," - 1)"],"")
#4/6/04              newlineend = string.join([newlineend,")"],"")
...
#           factor = "1.0d0"
      ...
      if (not fuse):
         ...
         newline = string.join(["IF (.not.MA_PUSH_GET(mt_dbl,dimc,'noname',l_c,k_c)) CALL ERRQUIT('",\
                                subroutinename,"',",repr(errquit),",MA_ERR)"],"")
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
...
#4/6/04     if (newline == ""):
#4/6/04        if (three):
#4/6/04           newline = string.join(["idimc_sort = ",sorted[nindex].show()],"")
#4/6/04        else:
#4/6/04           newline = string.join(["idima_sort = ",sorted[nindex].show()],"")
#4/6/04     else:
#4/6/04        newline = string.join([newline," + int_mb(k_range+",sorted[nindex-1].show(),"b-1)"\
...
#4/6/04  for nindex in range(len(permutedindexes)-1,-1,-1):
#4/6/04     if (newline == ""):
#4/6/04        newline = string.join(["idimc = ",permutedindexes[nindex].show()],"")
#4/6/04     else:
#4/6/04        newline = string.join([newline," + int_mb(k_range+",permutedindexes[nindex+1].show(),"b-1)"\
#4/6/04                               " * ((",permutedindexes[nindex].show()," - 1)"],"")
#4/6/04        newlineend = string.join([newlineend,")"],"")
...
#4/6/04        newline = "dbl_mb(k_c + idimc - 1) = - dbl_mb(k_c_sort + idimc_sort - 1)"
#4/6/04     else:
#4/6/04        newline = string.join(["dbl_mb(k_c + idimc - 1) = ",\
...
#4/6/04  newcode.pointer = newcode.pointer + 1
         ...
         for nindex in range(len(permutedindexes)-1,-1,-1):
            if (permutedindexes[nindex].type == "hole"):
               ...
               boffset = "b - noab - 1"
            if (arguments == ""):
               arguments = string.join(["d_c,dbl_mb(k_c),dimc,int_mb(k_c_offset),(",permutedindexes[nindex].show(),boffset],"")
            else:
               if (permutedindexes[nindex+1].type == "hole"):
                  arguments = string.join([arguments," + noab * (",permutedindexes[nindex].show(),boffset],"")
               else:
               ...
               argumentsend = string.join([argumentsend,")"],"")
         if (arguments == ""):
            arguments = "d_c,dbl_mb(k_c),dimc,int_mb(k_c_offset),0"
         else:
            arguments = string.join([arguments,argumentsend,")"],"")
         newline = string.join(["CALL ADD_HASH_BLOCK(",arguments,")"],"")
         if (not fuse):
            newcode.statements.insert(newcode.pointer,newline)
         if (not ifblock):
            newcode.setamark(3)
...
#3/24/2004 end
   ...
   def isoc99(self,globaltargetindexes,types,subroutinename="NONAME",fuse=0):
      """Suggests an implementation in ISO C99 for an elementary tensor contraction C = A * B"""
      ...
      language = "ISOC99"
      newcode = Code(language,subroutinename)

      # header
      newcode.add("headers",'/* ElementaryTensorContraction:')
      newcode.add("headers",' * '+self.show(0))
      newcode.add("headers",' */' + ln())
      ...
      # loop over output tensor indexes
      newcode.inserttiledforloops(superglobalone)
      if (three):
         newcode.inserttiledforloops(superglobaltwo)
      newcode.inserttiledforloops(superlocalone)
      if (three):
         newcode.inserttiledforloops(superlocaltwo)
      newcode.inserttiledforloops(subglobalone)
      if (three):
         newcode.inserttiledforloops(subglobaltwo)
      newcode.inserttiledforloops(sublocalone)
      if (three):
         newcode.inserttiledforloops(sublocaltwo)

      if (fuse):
         newcode.add("logicals","skipped")
         newline = "skipped = true;" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         superlocalparticleone  = []
         superlocalparticletwo  = []
         ...
         for npermutation in range(len(currentfactor.permutations)):
            ...
            for nindex in range(len(permutedindexes)):
               indexa = "t_" + self.tensors[0].indexes[nindex].show() + "b"
               indexb = permutedindexes[nindex].show() + "b"
               newline = newline + conjugation + "("+indexa+" == "+indexb+")"
               conjugation = " && "
            newline = newline + ") skipped = false;" + ln()
            newcode.statements.insert(newcode.pointer,newline)
            newcode.pointer = newcode.pointer + 1

         newline = "if(!skipped) {" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.setamark(4)
         newcode.pointer = newcode.pointer + 1
         newline = "}"
      ...
      # allocate tensor 0 tiles
      newcode.add("integers","dimc")
      newcode.add("locals","dimc")
      newline = ""
      for index in superglobalzero+superlocalzero+subglobalzero+sublocalzero:
         if (newline == ""):
            newline = "dimc = k_range["+index.show()+"b]"
         else:
            newline = newline+" * k_range["+index.show()+"b]"
      if (newline == ""):
         newline = "dimc = 1"
      newline = newline + ';' + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      if (three):
         newcode.add("doublearrays","k_c_sort")
         newcode.add("locals","k_c_sort")
         newline = "k_c_sort = tce_double_malloc(dimc);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         newline = "memset(k_c_sort,0,dimc*sizeof(*k_c_sort));" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1

         # loop over summation indexes
         newcode.inserttiledforloops(supercommon)
         newcode.inserttiledforloops(subcommon)
      ...
      indexesone = superglobalone + superlocalone + supercommonone + subglobalone + sublocalone + subcommonone
      if (indexesone):
         newline = "tce_restricted_"+repr(len(indexesone))+"("
         conjugation = ""
         for index in indexesone:
            newint = string.join([index.show(),"b"],"")
            newline = newline + conjugation + newint
      ...
      # spin restriction on input tensor two
      if (three):
         indexestwo = superglobaltwo + superlocaltwo + supercommontwo + subglobaltwo + sublocaltwo + subcommontwo
         if (indexestwo):
            newline = "tce_restricted_"+repr(len(indexestwo))+"("
            conjugation = ""
            for index in indexestwo:
               newint = string.join([index.show(),"b"],"")
               newline = newline + conjugation + newint
      ...
      # create MA's for tensor 1
      newcode.add("integers","dim_common")
      newcode.add("locals","dim_common")
      newline = ""
      for index in supercommonone + subcommonone:
         if (newline == ""):
            newline = string.join(["dim_common = k_range[",index.show(),"b]"],"")
         else:
            newline = string.join([newline," * k_range[",index.show(),"b]"],"")
      if (newline == ""):
         newline = "dim_common = 1"
      ...
      newcode.pointer = newcode.pointer + 1
      newcode.add("integers","dima_sort")
      newcode.add("locals","dima_sort")
      newline = ""
      for index in superglobalone + superlocalone + subglobalone + sublocalone:
         if (newline == ""):
            newline = string.join(["dima_sort = k_range[",index.show(),"b]"],"")
         else:
            newline = string.join([newline," * k_range[",index.show(),"b]"],"")
      if (newline == ""):
         newline = "dima_sort = 1"
      ...
      newcode.pointer = newcode.pointer + 1
      newcode.add("integers","dima")
      newcode.add("locals","dima")
      newline = "dima = dim_common * dima_sort;" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      ...
      if (three):
         newcode.add("integers","dimb_sort")
         newcode.add("locals","dimb_sort")
         newline = ""
         for index in superglobaltwo + superlocaltwo + subglobaltwo + sublocaltwo:
            if (newline == ""):
               newline = string.join(["dimb_sort = k_range[",index.show(),"b]"],"")
            else:
               newline = string.join([newline," * k_range[",index.show(),"b]"],"")
         if (newline == ""):
            newline = "dimb_sort = 1"
         ...
         newcode.pointer = newcode.pointer + 1
         newcode.add("integers","dimb")
         newcode.add("locals","dimb")
         newline = "dimb = dim_common * dimb_sort;" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1

      if (three):
         newline = "if(!((dima > 0) && (dimb > 0))) continue;" + ln()
      else:
         newline = "if(!(dima > 0)) continue;" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1

      # allocate sorted and unsorted tensor 1
      newcode.add("doublearrays","k_a_sort")
      newcode.add("locals","k_a_sort")
      newline = "k_a_sort = tce_double_malloc(dima);" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      errquit = errquit + 1
      newcode.add("doublearrays","k_a")
      newcode.add("locals","k_a")
      newline = "k_a = tce_double_malloc(dima);" + ln()
      newcode.statements.insert(newcode.pointer,newline)
      newcode.pointer = newcode.pointer + 1
      errquit = errquit + 1
         ...
         if (superpermutation[1] != "empty"):
            for nindex in range(len(superpermutedindexes)-1):
               ...
               if (indexa.isgreaterthan(indexb)):
                  ...
                  inequality = " <= "
               if (superline):
                  superline = string.join([superline," && (",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
               else:
                  superline = string.join([superline,"if((",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
         for subpermutation in subpermutations:
            subline = superline
            ...
            if (subpermutation[1] != "empty"):
               for nindex in range(len(subpermutedindexes)-1):
                  ...
                  if (indexa.isgreaterthan(indexb)):
                     ...
                     inequality = " <= "
                  if (subline):
                     subline = string.join([subline," && (",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
                  else:
                     subline = string.join([subline,"if((",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
            if (subline):
               subline = string.join([subline,") {",ln()],"")
               newcode.pointer = newcode.getamark(1) + 1
               newcode.statements.insert(newcode.pointer,subline)
               newcode.pointer = newcode.pointer + 1
               newline = "}" + ln()
            ...
            for nindex in range(len(permutedindexes)-1,-1,-1):
               if (permutedindexes[nindex].type == "hole"):
                  ...
                  if (self.tensors[1].type in types[4]):
                     ...
                     boffset = "b_1 - noab"
               indexl = string.join([indexl,",",permutedindexes[nindex].show(),boffset],"")
               if (indexc == ""):
                  indexc = string.join([",(",permutedindexes[nindex].show(),boffset],"")
               else:
                  if (self.tensors[1].type in types[4]):
                     indexc = string.join([indexc," + (noab+nvab) * (",permutedindexes[nindex].show(),boffset],"")
                  else:
                     if (permutedindexes[nindex+1].type == "hole"):
                        indexc = string.join([indexc," + noab * (",permutedindexes[nindex].show(),boffset],"")
                     else:
                        indexc = string.join([indexc," + nvab * (",permutedindexes[nindex].show(),boffset],"")
                  indexcend = string.join([indexcend,")"],"")
            if (not indexc):
               indexc = ",0"
            else:
               indexc = string.join([indexc,indexcend,")"],"")

            if(self.tensors[1].type == "v"):
               newcode.statements.insert(newcode.pointer,string.join(["if(intorb) {",ln()],""))
               newcode.pointer = newcode.pointer + 1
               newline = string.join(["tce_get_block_ind_i(",arguments,indexl,");",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newcode.statements.insert(newcode.pointer,string.join(["} else {",ln()],""))
               newcode.pointer = newcode.pointer + 1

            newline = string.join(["tce_get_hash_block(",arguments,",k_a_offset",indexc,");",ln()],"")
            newcode.statements.insert(newcode.pointer,newline)
            newcode.pointer = newcode.pointer + 1

            if(self.tensors[1].type == "v"):
               newcode.statements.insert(newcode.pointer,string.join(["}",ln()],""))
               newcode.pointer = newcode.pointer + 1

            # sort indexes of tensor 1
            newline = "tce_sort_"+repr(len(permutedindexes))+"(k_a,k_a_sort,"
            for nindex in range(len(permutedindexes)):
               index = permutedindexes[nindex]
               newint = index.show()+"b"
               newline = newline + "k_range[" + newint + "],"
            sorted = supercommonone + subcommonone + superglobalone + superlocalone + subglobalone + sublocalone
            for nindex in range(len(sorted)-1,-1,-1):
               index = sorted[nindex]
               for mindex in range(len(permutedindexes)):
                  if (index.isidenticalto(permutedindexes[mindex])):
                     newline = newline + repr(mindex) + ","
            if (superfactor * subfactor == 1):
               newline = newline + "1.0);" + ln()
            else:
               newline = newline + "-1.0);" + ln()
            newcode.statements.insert(newcode.pointer,newline)
            if (ifblock):
      ...
      # allocate sorted and unsorted tensor 2
      if (three):
         newcode.add("doublearrays","k_b_sort")
         newcode.add("locals","k_b_sort")
         newline = "k_b_sort = tce_double_malloc(dimb);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         newcode.add("doublearrays","k_b")
         newcode.add("locals","k_b")
         newline = "k_b = tce_double_malloc(dimb);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         ...
         for superpermutation in superpermutations:
            ...
            for subpermutation in subpermutations:
               ...
               newline = string.join(["tce_get_hash_block(",arguments,",k_b_offset",indexc,");",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               ...
               # sort indexes of tensor 2
               newline = "tce_sort_"+repr(len(permutedindexes))+"(k_b,k_b_sort,"
               for nindex in range(len(permutedindexes)):
                  index = permutedindexes[nindex]
                  newint = index.show()+"b"
                  newline = newline + "k_range[" + newint + "],"
         ...
         if (len(supercommonhole) > 1):
            ...
            newcode.pointer = newcode.pointer + 1
            for nindex in range(len(supercommonhole)-1):
               newline = string.join(["if(",supercommonhole[nindex].show(),"b == ",supercommonhole[nindex+1].show(),"b) {",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "nsuperh(isuperh) = nsuperh(isuperh) + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "} else {" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "isuperh = isuperh + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "}" + ln()
               newcode.statements.insert(newcode.pointer,newline)
         ...
         if (len(supercommonparticle) > 1):
            ...
            newcode.add("locals",newint)
            for isuperp in range(len(supercommonparticle)):
               newline = "nsuperp["+repr(isuperp)+"] = 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
            newline = "isuperp = 0;" + ln()
            newcode.statements.insert(newcode.pointer,newline)
            newcode.pointer = newcode.pointer + 1
            for nindex in range(len(supercommonparticle)-1):
               newline = string.join(["if(",supercommonparticle[nindex].show(),"b == ",supercommonparticle[nindex+1].show(),"b) {",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "nsuperp[isuperp]++;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "} else {" + ln()
               newcode.statements.insert(newcode.pointer,newline)
         ...
         if (len(subcommonhole) > 1):
            ...
            newcode.pointer = newcode.pointer + 1
            for nindex in range(len(subcommonhole)-1):
               newline = string.join(["if(",subcommonhole[nindex].show(),"b == ",subcommonhole[nindex+1].show(),"b) {",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "nsubh[isubh] = nsubh[isubh] + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "} else {" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "isubh = isubh + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "}" + ln()
               newcode.statements.insert(newcode.pointer,newline)
         ...
         if (len(subcommonparticle) > 1):
            ...
            newcode.add("integers",newint)
            for isubp in range(len(subcommonparticle)):
               newline = "nsubp["+repr(isubp+1)+"] = 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
            newline = "isubp = 1;" + ln()
            newcode.statements.insert(newcode.pointer,newline)
            newcode.pointer = newcode.pointer + 1
            for nindex in range(len(subcommonparticle)-1):
               newline = string.join(["if(",subcommonparticle[nindex].show(),"b == ",subcommonparticle[nindex+1].show(),"b) {",ln()],"")
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "nsubp[isubp] = nsubp[isubp] + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "} else {"
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "isubp = isubp + 1;" + ln()
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "}" + ln()
               newcode.statements.insert(newcode.pointer,newline)
         ...
         if (factorialforsuperhole or factorialforsuperparticle or factorialforsubhole or factorialforsubparticle):
            factor = repr(factorial(len(supercommonparticle))*factorial(len(supercommonhole))* \
                          factorial(len(subcommonparticle))*factorial(len(subcommonhole)))+".0"
            if (factorialforsuperhole):
               for isuperh in range(len(supercommonhole)):
                  factor = factor + "/FACTORIAL[nsuperh["+repr(isuperh)+"]]"
            if (factorialforsuperparticle):
               for isuperp in range(len(supercommonparticle)):
                  factor = factor + "/FACTORIAL[nsuperp["+repr(isuperp)+"]]"
            if (factorialforsubhole):
               for isubh in range(len(subcommonhole)):
                  factor = factor + "/FACTORIAL[nsubh["+repr(isubh)+"]]"
            if (factorialforsubparticle):
               for isubp in range(len(subcommonparticle)):
                  factor = factor + "/FACTORIAL[nsubp["+repr(isubp)+"]]"
         else:
            factor = "1.0"
         # TO HERE
  
         # perform contraction
         newline = string.join(["TCE_DGEMM(CblasTrans,CblasNoTrans,dima_sort,dimb_sort,dim_common,",factor,\
                                ",k_a_sort,dim_common,k_b_sort,dim_common,1.0,k_c_sort,dima_sort);",ln()],"")
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         newline = "tce_free(k_b_sort);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         newline = "tce_free(k_a_sort);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
         newcode.pointer = newcode.getamark(5)
      ...
      # create an MA for unsorted tensor 0
      if (not fuse):
         newcode.add("doublearrays","k_c")
         newcode.add("locals","k_c")
         newline = "k_c = tce_double_malloc(dimc);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
      ...
      # ... TO HERE
         ...
         if (not fuse):
            ...
            for nindex in range(len(permutedindexes)-1):
               ...
               inequality = " <= "
               if (newline):
                  newline = string.join([newline," && (",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
               else:
                  newline = string.join(["if((",indexa.show(),"b",inequality,indexb.show(),"b)"],"")
            if (newline):
               newline = string.join([newline,") {",ln()],"")
               newcode.pointer = newcode.getamark(3) + 1
               newcode.statements.insert(newcode.pointer,newline)
               newcode.pointer = newcode.pointer + 1
               newline = "}" + ln()
         ...
         if (not fuse):
            if (three):
               newline = "tce_sort_"+repr(len(permutedindexes))+"(k_c_sort,k_c,"
            else:
               newline = "tce_sort_"+repr(len(permutedindexes))+"(k_a_sort,k_c,"
         else:
            if (three):
               newline = "tce_sortacc_"+repr(len(permutedindexes))+"(k_c_sort,a_c,"
            else:
               newline = "tce_sortacc_"+repr(len(permutedindexes))+"(k_a_sort,a_c,"
         sorted = superglobalone + superlocalone + subglobalone + sublocalone \
                + superglobaltwo + superlocaltwo + subglobaltwo + sublocaltwo
         for nindex in range(len(sorted)-1,-1,-1):
            index = sorted[nindex]
         ...
         for nindex in range(len(permutedindexes)):
            ...
            for mindex in range(len(sorted)):
               if (index.isidenticalto(sorted[mindex])):
                  newline = newline + repr(len(sorted)-mindex-1) + ","
         if (currentfactor.coefficients[npermutation] == 1.0):
            newline = newline + "1.0);" + ln()
         elif (currentfactor.coefficients[npermutation] == - 1.0):
            newline = newline + "-1.0);" + ln()
         else:
            newline = newline + repr(rationaltofractional(currentfactor.coefficients[npermutation])[0]) + ".0/" \
                              + repr(rationaltofractional(currentfactor.coefficients[npermutation])[1]) + ".0);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         doloop = 0
         if (ifblock):
            newcode.setamark(4)
         ...
         for nindex in range(len(permutedindexes)-1,-1,-1):
            if (permutedindexes[nindex].type == "hole"):
               ...
               boffset = "b - noab"
            if (arguments == ""):
               arguments = string.join(["d_c,k_c,dimc,k_c_offset,(",permutedindexes[nindex].show(),boffset],"")
            else:
               if (permutedindexes[nindex+1].type == "hole"):
                  arguments = string.join([arguments," + noab * (",permutedindexes[nindex].show(),boffset],"")
               else:
               ...
               argumentsend = string.join([argumentsend,")"],"")
         if (arguments == ""):
            arguments = "d_c,k_c,dimc,k_c_offset[0],0"
         else:
            arguments = string.join([arguments,argumentsend,")"],"")
         newline = string.join(["tce_add_hash_block(",arguments,");",ln()],"")
         if (not fuse):
            newcode.statements.insert(newcode.pointer,newline)
         if (not ifblock):
            newcode.setamark(3)
      ...
      newcode.pointer = newcode.getamark(3) + 1
      if (not fuse):
         newline = "tce_free(k_c);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
      if (three):
         newline = "tce_free(k_c_sort);" + ln()
         newcode.statements.insert(newcode.pointer,newline)
         newcode.pointer = newcode.pointer + 1
         errquit = errquit + 1
      else:
...
class OperationTree:
   ...
   def tex(self,symbols={},verbose=1):
      ...
      show = []
      show.append("\\begin{align}")
      show = show + self.texa(symbols,verbose)
      # remove the trailing '\\' from the final line
      originallength = len(show[len(show)-1])
      show[len(show)-1] = show[len(show)-1][0:originallength-2]
      show.append("\\end{align}")
      return show

   def texa(self,symbols={},verbose=1):
      """Returns the contents as a list of strings in LaTeX format"""
      show = []
      show.append("% OT.texa begin")
      intermediatelist = ['r','\\chi','\\xi','\\kappa','\\eta','\\zeta','\\gamma','\\theta','\\lambda','\\pi','\\sigma']
#      show.append("% OT.texa begin")
      if (self.contraction.isoperation()):
#         show.append("% OT.texa contraction is a " + self.contraction.__class__.__name__)
         show.append(string.join([self.contraction.tex(symbols,verbose),"\\nonumber\\\\"],""))
      if (self.sisters):
         show.append("% OT.texa sisters:")
         for sister in self.sisters:
            show.append(string.join([sister.tex(symbols,verbose),"\\nonumber\\\\"],""))
      for child in self.children:
         if (child.isoperation()):
            show.append("% OT.texa: child is a " + child.__class__.__name__)
            show = show + child.texa(symbols,verbose)
      show.append("% OT.texa end")
      return show
...
#3/24/2004 end
   ...
   def fortran77(self,filename="NONAME",excitation=[],deexcitation=[],intermediate=[],general=[],fuse=0):
      ...
      # loop over the tree
      newcode.join(selfcopy.fortran77a(filename,"",globaltargetindexes,types,callees,fuse).expand())
...
#        newlistofcodes.add(antisymmetrizer)
   ...
   def fortran77a(self,subroutinename,name,globaltargetindexes,types,callees,fuse):
      """Returns a part of program that is generated by recursively interpreting the tree"""

      newcode = Code("Fortran77",subroutinename+name)
      ...
      if (self.sisters):
         ...
         for sister in self.sisters:
            counter = counter + 1
            newname = string.join([name,"__",repr(counter)],"")
            ...
            callee = sister.fortran77(globaltargetindexes,types,subroutinename+newname,0)
            callees.add(callee)
            ...
            # dump the caller
            newline = string.join(["CALL OFFSET_",subroutinename+newname,"(",l_c_offset,",",k_c_offset,",",size_c,")"],"")
            newcode.statements.insert(0,newline)
            newchar = "filename"
            newcode.add("characters",newchar)
            filename = string.join([subroutinename+newname,"_j",repr(sister.tensors[0].label)],"")
...
#           newcode.statements.insert(0,newline)
#           callee = sister.tensors[0].fortran77y(globaltargetindexes,types,subroutinename+newname)
            callee = sister.tensors[0].fortran77z(globaltargetindexes,types,subroutinename+newname)
            callees.add(callee)
            ...
            newline = string.join(["CALL ",subroutinename+newname,"(",argument,")"],"")
            newcode.statements.insert(0,newline)
...
#           newcode.add("arguments",size_c)
         ...
         if (not self.contraction.isoperation()):
            d_c = string.join(["a_i",repr(self.children[0].contraction.tensors[0].label)],"")
            newcode.add("doublearrays",d_c)
            newcode.add("arguments",d_c)
...
#4/17/04
      ...
      for nchild in range(len(self.children)):
         ...
         if (child.contraction.isoperation()):
            counter = counter + 1
            newname = string.join([name,"_",repr(counter)],"")
            ...
            if ((not fuse) or (self.contraction.isoperation())):
               callee = child.contraction.fortran77(globaltargetindexes,types,subroutinename+newname,0)
            else:
               callee = child.contraction.fortran77(globaltargetindexes,types,subroutinename+newname,1)
            callees.add(callee)
            ...
            if (createfile):
               if (not fuse):
                  ...
                  newchar = "filename"
                  newcode.add("characters",newchar)
                  filename = string.join([subroutinename+newname,"_i",repr(child.contraction.tensors[0].label)],"")
                  newline = string.join(["CALL TCE_FILENAME('",filename,"',filename)"],"")
                  newcode.statements.insert(0,newline)
                  newline = string.join(["CALL CREATEFILE(filename,",d_c,",",size_c,")"],"")
                  newcode.statements.insert(0,newline)
#                 callee = child.contraction.tensors[0].fortran77y(globaltargetindexes,types,subroutinename+newname)
                  callee = child.contraction.tensors[0].fortran77z(globaltargetindexes,types,subroutinename+newname)
                  callees.add(callee)
                  createfile = 0
               else:
                  depth = self.children[0].contraction.tensors[0].label
                  ...
                  newcode.add("integers",newint)
                  newline = string.join(["IF (toggle .eq. 1) CALL OFFSET_",subroutinename+newname,"(",l_c_offset,",",k_c_offset,",",size_c,")"],"")
                  newcode.statements.insert(0,newline)
                  newchar = "filename"
                  newcode.add("characters",newchar)
                  filename = string.join([subroutinename+newname,"_i",repr(child.contraction.tensors[0].label)],"")
                  newline = string.join(["IF (toggle .eq. 1) CALL TCE_FILENAME('",filename,"',filename)"],"")
                  newcode.statements.insert(0,newline)
                  newline = string.join(["IF (toggle .eq. 1) CALL CREATEFILE(filename,",d_c,",",size_c,")"],"")
                  newcode.statements.insert(0,newline)
#                 callee = child.contraction.tensors[0].fortran77y(globaltargetindexes,types,subroutinename+newname)
                  callee = child.contraction.tensors[0].fortran77z(globaltargetindexes,types,subroutinename+newname)
                  callees.add(callee)
                  createfile = 0
            newcode.statements.insert(0,child.fortran77a(subroutinename,newname,globaltargetindexes,types,callees,fuse))
...
#           newcode.statements.insert(0,newline)
            if (child.contraction.tensors[1].type == "i"):
               if (not fuse):
                  ...
                  newline = string.join(["IF (toggle .eq. 1) CALL RECONCILEFILE(",d_a,",",size_a,")"],"")
                  newcode.statements.insert(0,newline)
            if (d_b):
               if (child.contraction.tensors[2].type == "i"):
                  if (not fuse):
                     ...
                     newline = string.join(["IF (toggle .eq. 1) CALL RECONCILEFILE(",d_b,",",size_b,")"],"")
                     newcode.statements.insert(0,newline)
            argument = string.join([d_a,",",k_a_offset],"")
            if (d_b):
               argument = string.join([argument,",",d_b,",",k_b_offset],"")
#4/17/04
            if ((not fuse) or (self.contraction.isoperation())):
               ...
               argument = string.join([argument,",",d_c],"")
...
#4/17/04
            if (not fuse):
               ...
               if (depth):
                  newline = string.join(["IF (toggle .eq. 1) CALL ",subroutinename+newname,"(",argument,")"],"")
               else:
                  newline = string.join(["IF (toggle .eq. 2) CALL ",subroutinename+newname,"(",argument,")"],"")
               newcode.statements.insert(0,newline)
#           newline = string.join(["CALL GATODRA(",d_a,")"],"")
#           newcode.statements.insert(0,newline)
#           if (d_b):
...
#           newcode.statements.insert(0,newline)
      ...
      if (self.sisters):
         ...
         for isister in range(len(self.sisters)-1,-1,-1):
            ...
            l_c_offset = string.join(["l_j",repr(sister.tensors[0].label),"_offset"],"")
            k_c_offset = string.join(["k_j",repr(sister.tensors[0].label),"_offset"],"")
            newname = string.join([name,"_r_",repr(counter)],"")
...
#        newcode.add("arguments",size_c)
   ...
   def pythongen(self,filename="NONAME"):
      ...
      pythoncode.append(newline)
      newline = " # This is a Python program generated by Tensor Contraction Engine v.1.0.ETI"
      pythoncode.append(newline)
      newline = " # (c) All rights reserved by Battelle & Pacific Northwest Nat'l Lab (2002)"
      pythoncode.append(newline)
      ...
      newline = string.join([" return ",self.tensorslist([])[0]],"")
      pythoncode.append(newline)
      ...
      for child in self.children:
         if (child.contraction.isoperation()):
            ...
            newline = string.join([child.contraction.tensors[0].pythongen(),"=",child.contraction.tensors[0].pythongen(),"+",\
                                   "(",child.contraction.factor.show(0),")"],"")
...
# ... TO HERE
   ...
   def isoc99(self,filename="NONAME",excitation=[],deexcitation=[],intermediate=[],general=[],fuse=0):
      """Suggests an implementation in ISO C99 for the whole operation tree"""

      print(" ... generating ISO C99 code")
      print " "
      all = excitation+deexcitation+intermediate+general
      if (not all):
         types = self.tensortypes()
      ...
      for type in types[0]:
         if (type in types[1]):
            print " '"+type+"' is an excitation tensor"
            if ((type == 'v') and (not all)):
               raise RuntimeError, "unusual naming convention"
            if ((type == 'f') and (not all)):
               raise RuntimeError, "unusual naming convention"
            ...
            print " '"+type+"' is a de-excitation tensor"
            if ((type == 'v') and (not all)):
               raise RuntimeError, "unusual naming convention"
            if ((type == 'f') and (not all)):
               raise RuntimeError, "unusual naming convention"
      ...
      language = "ISOC99"
      newcode = Code(language,filename)
      ...
      # loop over the tree
      newcode.join(selfcopy.isoc99_function(filename,"",globaltargetindexes,types,callees,fuse).expand())

      # antisymmetrizer
      newcode.pointer = len(newcode.statements)

      # HACK.  Turn the list upside down.  This means the callees are in front
      # of the callers, so there's no implicit prototype issues with the
      # compiler.  It also means the LAST function gets the tce.h #include.
      callees.list.reverse()
      newlistofcodes.join(callees)
      newlistofcodes.list[0].add("includes",'#include "tce.h"' + ln())
      newlistofcodes.add(newcode)
   ...
   def isoc99_function(self,subroutinename,name,globaltargetindexes,types,callees,fuse):
      """Returns a part of program that is generated by recursively interpreting the tree"""

      language = "ISOC99"
      newcode = Code(language,subroutinename+name)
      ...
      if (self.sisters):
         ...
         for sister in self.sisters:
            ...
            k_c_offset = string.join(["k_j",repr(sister.tensors[0].label),"_offset"],"")
            size_c = string.join(["size_j",repr(sister.tensors[0].label)],"")
            newcode.add("integerarrays",k_c_offset)
            newcode.add("integers",size_c)
 
            # generate the contraction callee
            callee = sister.isoc99(globaltargetindexes,types,subroutinename+newname,0)
            callees.add(callee)

            # Tensor 2
            d_a = string.join(["d_",sister.tensors[1].type,repr(len(sister.tensors[1].indexes)/2)],"") 
            newcode.add("doublearrays",d_a)
            newcode.add("arguments",d_a)
            k_a_offset = string.join(["k_",sister.tensors[1].type,\
                         repr(len(sister.tensors[1].indexes)/2),"_offset"],"")
            newcode.add("integerarrays",k_a_offset)
            newcode.add("arguments",k_a_offset)

            # Tensor 3
            d_b = string.join(["d_",sister.tensors[2].type,repr(len(sister.tensors[2].indexes)/2)],"") 
            newcode.add("doublearrays",d_b)
            newcode.add("arguments",d_b)
            k_b_offset = string.join(["k_",sister.tensors[2].type,\
                         repr(len(sister.tensors[2].indexes)/2),"_offset"],"")
            newcode.add("integerarrays",k_b_offset)
            newcode.add("arguments",k_b_offset)

            # dump the caller
            newline = string.join(["OFFSET_",subroutinename+newname,"(&",k_c_offset,",",size_c,");",ln()],"")
            newcode.statements.insert(0,newline)
            newchar = "filename"
            newcode.add("characters",newchar)
            filename = string.join([subroutinename+newname,"_j",repr(sister.tensors[0].label)],"")
            newline = string.join(["TCE_FILENAME('",filename,"',filename)"],"")
            newcode.statements.insert(0,newline)
            newline = string.join(["TCE_CREATEFILE(filename,",d_c,",",size_c,")"],"")
            newcode.statements.insert(0,newline)
            callee = sister.tensors[0].isoc99z(globaltargetindexes,types,subroutinename+newname)
            callees.add(callee)
            argument = string.join([d_a,",",k_a_offset],"")
            argument = string.join([argument,",",d_b,",",k_b_offset],"")
            argument = string.join([argument,",",d_c,",",k_c_offset],"")
            newline = string.join([subroutinename+newname,"(",argument,");",ln()],"")
            newcode.statements.insert(0,newline)
            newline = string.join(["TCE_RECONCILEFILE(",d_c,",",size_c,")"],"")
            newcode.statements.insert(0,newline)
 
      # get a buffer for intermediate storage
      if (not fuse):
         d_c = string.join(["d_i",repr(self.children[0].contraction.tensors[0].label)],"")
         newcode.add("doublearrays",d_c)
         if (not self.contraction.isoperation()):
      ...
      for nchild in range(len(self.children)):
         ...
         if (child.contraction.isoperation()):
            ...
            if ((not fuse) or (self.contraction.isoperation())):
               ...
               fuse_flag = 1
            callee = child.contraction.isoc99(globaltargetindexes,types,subroutinename+newname,fuse_flag)
            callees.add(callee)
            ...
            if (createfile):
               if (not fuse):
                  ...
                  newcode.add("integers",newint)
                  newcode.add("locals",newint)
                  newline = string.join([k_c_offset, " = OFFSET_",subroutinename+newname,"(&",size_c,");",ln()],"")
                  newcode.statements.insert(0,newline)
                  newline = string.join([d_c, " = tce_double_malloc(", size_c,");",ln()],"")
                  newcode.statements.insert(0,newline)
                  callee = child.contraction.tensors[0].isoc99z(globaltargetindexes,types,subroutinename+newname)
                  callees.add(callee)
                  createfile = 0
               else:
                  depth = self.children[0].contraction.tensors[0].label
                  ...
                  newcode.add("integers",newint)
                  newcode.add("locals",newint)
                  newline = string.join(["if(toggle == 1) ", k_c_offset, " = OFFSET_",subroutinename+newname,"(&",size_c,");",ln()],"")
                  newcode.statements.insert(0,newline)
                  newchar = "filename"
                  newcode.add("characters",newchar)
                  newline = string.join([d_c, " = tce_double_malloc(", size_c,");",ln()],"")
#                   filename = string.join([subroutinename+newname,"_i",repr(child.contraction.tensors[0].label)],"")
#                   newline = string.join(["if(toggle == 1) TCE_FILENAME('",filename,"',filename);",ln()],"")
#                   newcode.statements.insert(0,newline)
#                   newline = string.join(["if(toggle == 1) TCE_CREATEFILE(filename,",d_c,",",size_c,");",ln()],"")
                  newcode.statements.insert(0,newline)
                  callee = child.contraction.tensors[0].isoc99z(globaltargetindexes,types,subroutinename+newname)
                  callees.add(callee)
                  createfile = 0
            childcode = child.isoc99_function(subroutinename,newname,globaltargetindexes,types,callees,fuse)
            newcode.statements.insert(0,childcode)

#             if (child.contraction.tensors[1].type == "i"):
#                if (not fuse):
#                   newline = string.join(["TCE_RECONCILEFILE(",d_a,",",size_a,");",ln()],"")
#                   newcode.statements.insert(0,newline)
#                else:
#                   newline = string.join(["if(toggle == 1) TCE_RECONCILEFILE(",d_a,",",size_a,");",ln()],"")
#                   newcode.statements.insert(0,newline)
#             if (d_b):
#                if (child.contraction.tensors[2].type == "i"):
#                   if (not fuse):
#                      newline = string.join(["TCE_RECONCILEFILE(",d_b,",",size_b,");",ln()],"")
#                      newcode.statements.insert(0,newline)
#                   else:
#                      newline = string.join(["if(toggle == 1) TCE_RECONCILEFILE(",d_b,",",size_b,");",ln()],"")
#                      newcode.statements.insert(0,newline)
            arguments1 = [d_a]
            arguments2 = [k_a_offset]
            newline = subroutinename+newname
            if (d_b):
               arguments1.append(d_b)
               arguments2.append(k_b_offset)
            if ((not fuse) or (self.contraction.isoperation())):
               arguments1.append(d_c)
               arguments2.append(k_c_offset)
            else:
               arguments1.append[d_c]
               for index in child.contraction.tensors[0].indexes:
                  newint = "t_"+index.show()+"b"
                  arguments1.append(newint)
            arguments = arguments1 + arguments2
            newline = string.join([subroutinename, newname, "(", string.join(arguments,","), ");", ln()],"")
            if (fuse):
               depth = child.contraction.tensors[0].label
               if (depth):
                  newline = string.join(["if(toggle == 1) ",newline],"")
               else:
                  newline = string.join(["if(toggle == 2) ",newline],"")
            newcode.statements.insert(0,newline)
            ...
            if (child.contraction.tensors[1].type == "i"):
               if (fuse):
                  ...
                  if (depth == 1):
                     newline = "}" + ln()
                     newcode.statements.insert(len(newcode.statements),newline)
                     newline = string.join(["tce_free(",k_a_offset,");",ln()],"")
                     newcode.statements.insert(len(newcode.statements),newline)
                     newline = string.join(["TCE_DELETEFILE(",d_a,");",ln()],"")
                     newcode.statements.insert(len(newcode.statements),newline)
                     newline = "if(toggle == 3) {" + ln()
                     newcode.statements.insert(len(newcode.statements),newline)
                  elif (depth >= 2):
                     newline = "if(toggle == 1) {" + ln()
                     newcode.statements.insert(0,newline)
                     newline = string.join(["TCE_DELETEFILE(",d_a,");",ln()],"")
                     newcode.statements.insert(0,newline)
                     newline = string.join(["tce_free(",k_a_offset,");",ln()],"")
               ...
               if (child.contraction.tensors[2].type == "i"):
                  if (fuse):
                     ...
                     if (depth == 1):
                        newline = "}" + ln()
                        newcode.statements.insert(len(newcode.statements),newline)
                        newline = string.join(["tce_free(",k_b_offset,");",ln()],"")
                        newcode.statements.insert(len(newcode.statements),newline)
                        newline = string.join(["tce_free(",d_b,");",ln()],"")
                        newcode.statements.insert(len(newcode.statements),newline)
                        newline = "if(toggle == 3) {"
                        newcode.statements.insert(len(newcode.statements),newline)
                     elif (depth >= 2):
      ...
      if (self.sisters):
         ...
         for isister in range(len(self.sisters)-1,-1,-1):
            ...
            k_c_offset = string.join(["k_j",repr(sister.tensors[0].label),"_offset"],"")
            newname = string.join([name,"_r_",repr(counter)],"")
            newline = string.join(["tce_free(",d_c,")"],"")
            newcode.statements.insert(0,newline)
            newline = string.join(["tce_free(",l_c_offset,");"],"")
            newcode.statements.insert(0,newline)
...
class Code:
   ...
   def __init__(self,language,name):
      ...
      if (self.language == "Fortran77"):
         ...
         newline = "!This is a " + self.language + " program generated by Tensor Contraction Engine v.1.0.ETI"
         self.headers.append(newline)
         newline = "!Copyright (c) Battelle & Pacific Northwest National Laboratory (2002)"
         self.headers.append(newline)
      else:
         newline = self.comment + "$Id: tce.py,v 1.10 2002/12/01 21:37:34 sohirata Exp $"
         self.headers.append(newline)
         newline = self.comment + "This is a " + self.language + " program generated by Tensor Contraction Engine v.1.0.ETI"
         self.headers.append(newline)
         newline = self.comment + "Copyright (c) Battelle & Pacific Northwest National Laboratory (2002)"
         self.headers.append(newline)
   ...
   def expand(self):
      ...
      result = Code(language,self.name)
      ...
      for n in self.arglistpointers:
         result.add("arglistpointers",n)
      ...
      for n in self.locals:
         result.add("locals",n)
      for n in self.retvals:
         result.add("retvals",n)
      for n in self.tensors:
         result.add("tensors",n)
      for member in self.statements:
         if (isinstance(member,Code)):
   ...
   def setamark(self,number):
      ...
      while(keep_looking):
         keep_looking = 0
         # clean up previous instances of the same marker
         pos1 = statement.find("/*gotmarker"+repr(number))
         if(pos1 < 0):
            pos1 = statement.find("/*delmarker"+repr(number))
         if(pos1 < 0):
            pos1 = statement.find("/*delallmarker"+repr(number))
         if(pos1 > -1):
            pos2 = string.index(statement,"*/", pos1)
            if(pos1 > 0):
               statement = statement[0:pos1]+statement[pos2+2:]
            else:
               statement = statement[pos2+2:]
            keep_looking = 1

      # add marker
      statement = "/*marker"+repr(number)+":"+str(sys._getframe(1).f_lineno)+":"+repr(self.markopcount)+"*/"+statement
      self.markopcount = self.markopcount + 1
      self.statements[self.pointer]=statement

   def getamark(self,number):
      """Returns the position of an input marker"""
      substr = "/*marker"+repr(number)+":"
      for nstatement in range(len(self.statements)):
         statement = self.statements[nstatement]
         pos1 = statement.find(substr)
         if (pos1 > -1):
            if(pos1 > 0):
               statement = statement[0:pos1]+"/*got"+statement[pos1+2:]
            else:
               statement = "/*got"+statement[pos1+2:]
            self.statements[nstatement] = statement
            return nstatement
   ...
   def deleteamark(self,number):
      ...
      for nstatement in range(len(self.statements)):
         ...
         pos1 = statement.find(substr)
         if (pos1 > -1):
            number = int(string.split(statement[pos1+8:],":")[0])
            if(pos1 > 0):
               statement = statement[0:pos1]+"/*del"+statement[pos1+2:]
            else:
               statement = "/*del"+statement[pos1+2:]
            self.statements[nstatement] = statement
   ...
   def deleteallmarks(self):
      ...
      for nstatement in range(len(self.statements)):
         statement = self.statements[nstatement]
         pos1 = string.find(statement,"/*marker")
         while(pos1 > -1):
            number = int(string.split(statement[pos1+8:],":")[0])
            if(pos1 > 0):
               statement = statement[0:pos1]+"/*delall"+statement[pos1+2:]
            else:
               statement = "/*delall"+statement[pos1+2:]
            self.statements[nstatement] = statement
            pos1 = statement.find("/*marker")
#            print("deleteallmarks: new string "+statement)

   def isoc99_type(self,var):
      type = ''
      if var in self.integers:
         type = 'int'
      if var in self.integerarrays:
         type = 'int*'
      if var in self.integerallocatables:
         raise Exception("ISOC99 generator can't handle allocatables")
      if var in self.doubles:
         type = 'double'
      if var in self.doublearrays:
         type = 'double*'
      if var in self.doubleallocatables:
         raise Exception("ISOC99 generator can't handle allocatables")
      if var in self.doublecomplexs:
         type = 'double _Complex'
      if var in self.doublecomplexarrays:
         type = 'double _Complex*'
      if var in self.doublecomplexallocatables:
         raise Exception("ISOC99 generator can't handle allocatables")
      if var in self.arglistpointers:
         type = 'argument_list_t*'
      if var in self.logicals:
         type = 'bool'
      if var in self.logicalarrays:
         type = 'bool*'
      if var in self.characters:
         raise Exception("ISOC99 generator can't handle characters")
      if var in self.tensors:
         type = 'tensor_t*'
      if var in self.blocks:
         type = 'tensor_block_desc_t*'
      if type == '':
         type = 'void'
      return type

   def isoc99_variable(self,var):
      type = self.isoc99_type(var)
      if var in self.externals:
         var = string.join(["extern", type, var],' ')
      else:
         var = string.join([type, var],' ')
      return var
...
#           show.insert(pointer,string.join([self.indent,"DOUBLE COMPLEX, ALLOCATABLE :: ",n,"(:)"],""))
         ...
         for n in self.statements:
            if n.find("END") == 0:
               indent = indent - 2;
            show.insert(pointer,string.join([" "*indent,n],""))
            if n.find("DO") == 0:
               indent = indent + 2;
            pointer = pointer + 1
         self.indent = ""
         ...
         for n in self.retvals:
            if retval == "void":
               retval = self.isoc99_type(n)
            else:
               raise Exception("ISOC99 functions can't have multiple retvals")

         subroutine = string.join([retval, " " ,self.name],'')
         if (self.arguments):
            subroutine = string.join([subroutine,'('],'')
            for n in range(len(self.arguments)):
               argument = self.arguments[n]
               argument = self.isoc99_variable(argument)

               if (n != 0):
                  argument = string.join([',',argument],'')
               subroutine = string.join([subroutine,argument],'')
            subroutine = string.join([subroutine,') {',ln()],'')
         else:
            subroutine = string.join([subroutine,'(void) {',ln()],'')
         show.insert(pointer,subroutine)
         pointer = pointer + 1
         self.indent = "    "
         subroutine = string.join([subroutine,') {',ln()],'')
         ...
         # add local and extern variables, grouped by type (and extern-ness)
         typemap = {}
         for local in sorted(self.locals + self.externals):
            if not local in self.arguments:
               type = self.isoc99_type(local)
               ptr = ''
               if type.endswith('*'):
                  offset = type.index('*')
                  ptr = string.join([ptr,type[offset:len(type)]],'')
                  type = type[0:offset]
               if local in self.externals:
                  type = string.join(["extern ",type],'')
               if type not in typemap:
                  typemap[type] = [string.join([ptr,local],'')]
               else:
                  typemap[type].append(string.join([ptr,local],''))
         for type in sorted(typemap.keys()):
            line = string.join(sorted(typemap[type]), ', ')
            line = string.join([self.indent,type," ",line,";",ln()],'')
            show.insert(pointer,line)
            pointer = pointer + 1
         ...
         for n in self.statements:
            n_nocomment = n
            if n_nocomment.rfind(" //"):
               n_nocomment = n_nocomment.split(" //",1)[0]
            keep_checking = 1
            while(keep_checking):
               keep_checking = 0
               pos = n_nocomment.find("/*")
               if(pos > -1):
                  n_nocomment = n_nocomment[0:pos] + n_nocomment[n_nocomment.find("*/")+2:]
                  keep_checking = 1
            if n_nocomment.rfind('}') == 0:
               indent = indent - 4
            show.insert(pointer,string.join([" "*indent,n],""))
            if n_nocomment.rfind('{') == len(n_nocomment) - 1:
               indent = indent + 4
            pointer = pointer + 1

         if(len(self.retvals)):
            show.insert(pointer,self.indent+"return "+self.retvals[0]+";"+ln())
            pointer = pointer + 1

         show.insert(pointer, '}'+ln())
         pointer = pointer + 1
   ...
   def join(self,another):
      ...
      for n in another.arglistpointers:
         self.add("arglistpointers",n)
      ...
      for n in another.locals:
         self.add("locals",n)
      for n in another.retvals:
         self.add("retvals",n)
      for n in another.tensors:
         self.add("tensors",n)
      for n in another.blocks:
         self.add("blocks",n)
   ...
   def add(self,towhat,what):
      ...
      elif (towhat == "arglistpointers"):
         redundant = 0
         for n in self.arglistpointers:
            if (n == what):
               redundant = 1
         if (not redundant):
            self.arglistpointers.append(what)
   ...
   def insertdoloop(self,index):
      ...
      if (index.type == 'hole'):
         ...
         self.add("locals",newint)
         if (self.language == "ISOC99"):
            newline = string.join([self.indent,"for(",newint,"=1;",newint,"<nocc;",newint,",++) {",ln()],"")
         else:
         ...
         self.add("locals",newint)
         if (self.language == "ISOC99"):
            newline = string.join([self.indent,"for(",newint,"=nocc+1;",newint,"<=N;",newint,"++) {",ln()],"")
         else:
         ...
         self.add("locals",newint)
         if (self.language == "ISOC99"):
            newline = string.join([self.indent,"for(",newint,"=1;",newint,"<N;",newint,"++) {",ln()],"")
         else:
...
# ... TO HERE
         ...
         if (self.language == "ISOC99"):
            newline = string.join(["if(",indexa.show(),">=",indexb.show(),") continue;",ln()],"")
         else:
   ...
   def inserttiledforloops(self,list):
      """Inserts a nested for-loop"""
      for nindex in range(len(list)):
         index = list[nindex]
         newint = string.join([index.show(),"b"],"")
         self.add("integers",newint)
         ...
         if (nindex == 0):
            if (index.type == 'hole'):
               newline = string.join(["for(",newint," = 0;",newint," < noab;",newint,"++) {",ln()],"")
            if (index.type == 'particle'):
               newline = string.join(["for(",newint," = noab;",newint," < noab+nvab;",newint,"++) {",ln()],"")
            self.statements.insert(self.pointer,newline)
            self.pointer = self.pointer + 1
            newline = "}" + ln()
            self.statements.insert(self.pointer,newline)
            ...
            if (index.type == 'hole'):
               if (previoustype == 'hole'):
                  newline = string.join(["for(",newint," = ",previousint,";",newint," < noab;",newint,"++) {",ln()],"")
               else:
                  raise ValueError, "non-canonical expression found"
            if (index.type == 'particle'):
               if (previoustype == 'hole'):
                  ...
                  newline = string.join(["for(",newint," = ",previousint,";",newint," < noab+nvab;",newint,"++) {",ln()],"")
            self.statements.insert(self.pointer,newline)
            self.pointer = self.pointer + 1
            newline = "}" + ln()
            self.statements.insert(self.pointer,newline)
   ...
   def inserttiledifrestricted(self,indexes):
      ...
      for index in indexes:
         ...
         if (self.language == "Fortran77"):
            ...
            newline = string.join([newline,conjugation,"k_spin[",newint,"]"],"")
         conjugation = "+"
      if (self.language == "Fortran77"):
         ...
         newline = string.join([newline," != ",repr(2*len(indexes)),"))) continue;",ln()],"")
      self.statements.insert(self.pointer,newline)
      self.pointer = self.pointer + 1
      if (self.language != "ISOC99"):
         newline = "END IF"
...
# ... TO HERE
         ...
         for index in sub:
            ...
            conjugation = "+"
         if (self.language == "ISOC99"):
            newline = string.join([newline,")) continue;",ln()],"")
            self.statements.insert(self.pointer,newline)
            self.pointer = self.pointer + 1
         else:
            newline = string.join([newline,") THEN"],"")
...
# ... TO HERE
         ...
         for nindex in range(len(all)-1):
            ...
            if (self.language == "Fortran77"):
               ...
               newline = string.join([newline,conjugation,"(k_sym[",newint,"]"],"")
               conjugation = "^"
         index = all[len(all)-1]
         newint = string.join([index.show(),"b"],"")
         if (self.language == "Fortran77"):
      ...
      if (len(irrep) == 1):
         if (self.language == "ISOC99"):
            newline = string.join([newline,"irrep_",irrep[0],")) continue;",ln()],"")
         else:
         ...
         for neach in range(len(irrep)-1):
            each = irrep[neach]
            if (self.language == "ISOC99"):
               newline = string.join([newline,conjugation,"(irrep_",each],"")
               conjugation = "^"
            else:
               newline = string.join([newline,conjugation,"ieor(irrep_",each],"")
               conjugation = ","
         newline = string.join([newline,conjugation,"irrep_",irrep[len(irrep)-1]],"")
         for neach in range(len(irrep)-1):
            newline = string.join([newline,")"],"")
         if (self.language == "ISOC99"):
            newline = string.join([newline,")) continue;",ln()],"")
...
# ... TO HERE
         ...
         if (self.language == "Fortran77"):
            ...
            newline = string.join(["if(",indexa.show(),"b <= ",indexb.show(),"b) {",ln()],"")
         self.statements.insert(self.pointer,newline)
         self.pointer = self.pointer + 1
         if(self.language == "ISOC99"):
            newline = "}" + ln()
...
class ListofCodes:
   ...
   def writetofile(self,filename):
      ...
      if (self.list[0].language == "Fortran77"):
         ...
         file = open(filename+".F90","w")
      elif (self.list[0].language == 'ISOC99'):
         file = open(filename+'.c','w')
      for code in self.list:
         for n in code.wrap():
            file.write(n)
